    ...
    <form action="{{ route('admin.kontak.update', $kontak->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="nama" name="nama" class="w-full p-2 border border-gray-300 rounded"
                required value="{{ old('nama', $kontak->nama ?? '') }}">
            @error('nama')
...
<p class="text-sm text-gray-600 mt-1">Gambar saat ini:</p>
    <img src="{{ asset('storage/' . $kontak->image) }}" alt="Gambar Kontak" class="w-32 h-auto rounded mt-1">            @endif
            @error('image')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
